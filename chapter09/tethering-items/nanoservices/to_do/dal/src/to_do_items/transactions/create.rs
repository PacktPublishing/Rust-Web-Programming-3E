...
pub trait SaveOne {
    fn save_one(item: NewToDoItem, user_id: i32) -> impl Future<Output = Result<ToDoItem, NanoServiceError>> + Send;
}
...
#[cfg(feature = "sqlx-postgres")]
async fn sqlx_postgres_save_one(item: NewToDoItem, user_id: i32) -> Result<ToDoItem, NanoServiceError> {
