...
pub async fn update<T, X>(token: HeaderToken, Json(body): Json<ToDoItem>) -> Result<impl IntoResponse, NanoServiceError> 
where
    T: UpdateOne + GetAll,
    X: GetUserSession
{
    ...
    let _ = update_core::<T>(body, user.user_id).await?;

    // Fetch all items after creation
    let all_items = get_all_core::<T>(user.user_id).await?;
