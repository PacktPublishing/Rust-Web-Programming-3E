...
pub async fn create<T, X>(
    token: HeaderToken, Json(body): Json<NewToDoItem>
    ...
    X: GetUserSession
{
    ...
    let _ = create_core::<T>(body.into(), session.user_id).await?;
    Ok((StatusCode::CREATED, Json(get_all_core::<T>(session.user_id).await?)))
}
