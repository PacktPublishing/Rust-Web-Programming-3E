...
async fn handle(req: Request<hyper::body::Incoming>) 
    ...
    let response = match (req.method(), segments.as_slice()) {
        ...
        (&Method::GET, ["api", "v1", "users", "login"]) => {
            api::auth::login::login::<SqlxPostGresDescriptor, RedisSessionDescriptor>(req).await
        }
        (&Method::GET, ["api", "v1", "users", "get"]) => {
            let token = extract_hyper_header_token!(&req);
            api::users::get::get_by_unique_id::<SqlxPostGresDescriptor>(token).await
        }
        _ => Ok(not_found()),
    };
    match response {
