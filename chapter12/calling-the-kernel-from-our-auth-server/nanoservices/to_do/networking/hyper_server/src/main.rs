...
async fn handle(req: Request<hyper::body::Incoming>) 
    ...
    let response = match (req.method(), segments.as_slice()) {
        (&Method::GET, ["api", "v1", "get", "all"]) => {
            ...
            api::basic_actions::get::get_all::<SqlxPostGresDescriptor, RedisSessionDescriptor>(token).await
        }
        (&Method::POST, ["api", "v1", "create"]) => {
            let token = extract_hyper_header_token!(&req);
            // Extract and parse the JSON body
            api::basic_actions::create::create::<SqlxPostGresDescriptor, RedisSessionDescriptor>(req, token).await
        }
        (&Method::PATCH, ["api", "v1", "update"]) => {
            let token = extract_hyper_header_token!(&req);
            // Extract and parse the JSON body
            api::basic_actions::update::update::<SqlxPostGresDescriptor, RedisSessionDescriptor>(req, token).await
        }
        (&Method::DELETE, ["api", "v1", "delete", name]) => {
            let token = extract_hyper_header_token!(&req);
            // Here `name` is the extracted name segment from the URL
            api::basic_actions::delete::delete_by_name::<SqlxPostGresDescriptor, RedisSessionDescriptor>(name, token).await
        }
        _ => Ok(not_found()),
    };
    match response {
