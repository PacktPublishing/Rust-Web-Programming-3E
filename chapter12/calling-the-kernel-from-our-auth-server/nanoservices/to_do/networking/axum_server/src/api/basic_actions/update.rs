...
pub async fn update<T: UpdateOne + GetAll, X: GetUserSession>(token: HeaderToken, Json(body): Json<ToDoItem>) -> Result<impl IntoResponse, NanoServiceError> {
    let session = X::get_user_session(token.unique_id).await?;
    // Call the core create function
    let _ = update_core::<T>(body, session.user_id).await?;

    // Fetch all items after creation
    let all_items = get_all_core::<T>(session.user_id).await?;
