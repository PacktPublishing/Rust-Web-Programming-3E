...
pub trait DeleteOne {
    fn delete_one(item: ToDoItem) -> impl Future<Output = Result<ToDoItem, NanoServiceError>> + Send;
}

#[cfg(feature = "sqlx-postgres")]
impl DeleteOne for SqlxPostGresDescriptor {
    fn delete_one(item: ToDoItem) -> impl Future<Output = Result<ToDoItem, NanoServiceError>> + Send {
        sqlx_postgres_delete_one(item)
    }
}
...
impl DeleteOne for JsonFileDescriptor {
    fn delete_one(item: ToDoItem) -> impl Future<Output = Result<ToDoItem, NanoServiceError>> + Send {
        json_file_delete_one(item)
    }
}


#[cfg(feature = "sqlx-postgres")]
async fn sqlx_postgres_delete_one(item: ToDoItem) -> Result<ToDoItem, NanoServiceError> {
...
#[cfg(feature = "json-file")]
async fn json_file_delete_one(item: ToDoItem) -> Result<ToDoItem, NanoServiceError> {
    let mut tasks = get_all::<ToDoItem>().unwrap_or_else(|_| HashMap::new());
    let to_do_item = tasks.remove(&item.title.to_string()).ok_or_else(|| {
