...
async fn main() -> Result<()> {
    ...
    let malloc = instance.get_typed_func::<(i32, i32), i32>(
        &mut store, "ns_malloc"
    ).unwrap();
    ...
    let entry_point = instance.get_typed_func::<(i32, i32), i32>(
        &mut store, "entry_point"
    ).unwrap();
    let ret = entry_point.call_async(
        &mut store, (input_data_ptr, serialized.len() as i32)
    ).await.unwrap();
    ...
    let result_struct = unsafe {
        &from_raw_parts::<ResultPointer>(
            result_buffer.as_ptr() as *const ResultPointer, 1
        )[0]
    };
    ...
    memory.read(
        &mut store, result_struct.ptr as usize, &mut output_buffer
    ).unwrap();
    ...
    free.call_async(
        &mut store, (input_data_ptr, serialized.len() as i32, 0)
    ).await.unwrap();
    free.call_async(
        &mut store, (result_struct.ptr, result_struct.len, 0)
    ).await.unwrap();
    free.call_async(
        &mut store, (ret, size_of::<ResultPointer>() as i32, 0)
    ).await.unwrap();
    Ok(())
}
